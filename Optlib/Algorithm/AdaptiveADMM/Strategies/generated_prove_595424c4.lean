...
def tau_seq (c p : ℝ) (n : ℕ) : ℝ := c / Real.rpow ((n : ℝ) + 2) p

theorem h_tau_summable (c p : ℝ) (hp : 1 < p) : Summable (tau_seq c p) := by
  simpa [tau_seq] using p_series_summable_template c p hp

def residual_ratio (r_norm_seq s_norm_seq : ℕ → ℝ) (eps : ℝ) (n : ℕ) : ℝ :=
  r_norm_seq n / max (s_norm_seq n) eps

def dir_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (n : ℕ) : ℤ :=
  if residual_ratio r_norm_seq s_norm_seq eps n > mu then 1
  else if residual_ratio r_norm_seq s_norm_seq eps n < 1 / mu then -1 else 0
...
lemma h_dir (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) :
  ...
  intro n
  by_cases h1 : residual_ratio r_norm_seq s_norm_seq eps n > mu
  · simp [dir_seq, h1]
  · by_cases h2 : residual_ratio r_norm_seq s_norm_seq eps n < 1 / mu
    · simp [dir_seq, h1, h2]
    · simp [dir_seq, h1, h2]
...
lemma h_update_equiv (tau : ℕ → ℝ) (dir : ℕ → ℤ) (r_norm_seq s_norm_seq : ℕ → ℝ)
    ...
    ∀ n rho, 0 < rho →
      update_fun tau dir r_norm_seq s_norm_seq n rho = rho * (1 + 2 * tau n) ∨
      update_fun tau dir r_norm_seq s_norm_seq n rho = rho / (1 + 2 * tau n) ∨
      update_fun tau dir r_norm_seq s_norm_seq n rho = rho * (1 + (1/2) * tau n) ∨
      update_fun tau dir r_norm_seq s_norm_seq n rho = rho / (1 + (1/2) * tau n) ∨
      update_fun tau dir r_norm_seq s_norm_seq n rho = rho := by
  intro n rho hρ_pos
  rcases h_dir n with h | h | h
  · left; simp [update_fun, h]
  · rcases lt_trichotomy (r_norm_seq n) (s_norm_seq n) with hlt | heq | hgt
    · right; right; right; left; simp [update_fun, h, hlt, not_lt.mpr (le_of_lt hlt)]
    · right; right; right; right; simp [update_fun, h, heq]
    · right; right; left; simp [update_fun, h, hgt]
