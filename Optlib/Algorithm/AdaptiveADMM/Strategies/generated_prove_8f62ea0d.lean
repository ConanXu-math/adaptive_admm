...
def protected_residual (r_norm_seq : ℕ → ℝ) (eps : ℝ) (n : ℕ) : ℝ :=
  max (r_norm_seq n) eps

-- residual balancing: dir_seq n = 1 (mul), 0 (keep), -1 (div)
def dir_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (n : ℕ) : ℤ :=
  if r_norm_seq n > mu_seq mu n * protected_residual s_norm_seq eps n then 1
  else if s_norm_seq n > mu_seq mu n * protected_residual r_norm_seq eps n then -1 else 0
...
lemma h_dir (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) :
  ...
  intro n
  by_cases h1 : r_norm_seq n > mu_seq mu n * protected_residual s_norm_seq eps n
  · simp [dir_seq, h1]
  · by_cases h2 : s_norm_seq n > mu_seq mu n * protected_residual r_norm_seq eps n
    · simp [dir_seq, h1, h2]
    · simp [dir_seq, h1, h2]
