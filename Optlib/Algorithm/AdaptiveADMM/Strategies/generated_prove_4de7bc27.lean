...
def mu_seq (mu : ℝ) (n : ℕ) : ℝ := 1 + (mu - 1) / ((n : ℝ) + 1)

def r_plus (r_norm_seq : ℕ → ℝ) (eps : ℝ) (n : ℕ) : ℝ := max (r_norm_seq n) eps

def s_plus (s_norm_seq : ℕ → ℝ) (eps : ℝ) (n : ℕ) : ℝ := max (s_norm_seq n) eps

def dir_seq (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) (n : ℕ) : ℤ :=
  if r_norm_seq n > mu_seq mu n * s_plus s_norm_seq eps n then 1
  else if s_norm_seq n > mu_seq mu n * r_plus r_norm_seq eps n then -1 else 0
...
lemma h_dir (mu eps : ℝ) (r_norm_seq s_norm_seq : ℕ → ℝ) :
  ...
  intro n
  by_cases h1 : r_norm_seq n > mu_seq mu n * s_plus s_norm_seq eps n
  · simp [dir_seq, h1]
  · by_cases h2 : s_norm_seq n > mu_seq mu n * r_plus r_norm_seq eps n
    · simp [dir_seq, h1, h2]
    · simp [dir_seq, h1, h2]
